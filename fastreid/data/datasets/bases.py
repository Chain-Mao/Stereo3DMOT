...
class Dataset(object):
    ...
    # 此处根据parse_data修改
    def parse_kitti_data(self, data):
        """Parses data list and returns the number of person IDs
        and the number of camera views.
        Args:
            data (list): contains tuples of (img_path(s), pid, camid)
        ...
        right_pids = set()
        for info in data:
            left_pids.add(info[0][1])
            right_pids.add(info[0][1])
        assert len(left_pids) == len(right_pids), "左目pid数量不等于右目"
        return len(left_pids)
...
class ImageDataset(Dataset):
    ...
    def show_train(self):
        num_train_pids = self.parse_kitti_data(self.train)
        num_train_cams = 2
    ...
    def show_test(self):
        num_query_pids = self.parse_kitti_data(self.query)
        num_query_cams = 2
        num_gallery_pids = self.parse_kitti_data(self.gallery)
        num_gallery_cams = 2
