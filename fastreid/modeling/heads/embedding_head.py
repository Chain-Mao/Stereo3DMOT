...
class EmbeddingHead(nn.Module):
    ...
    def __init__(
        ...
        self.fc = nn.Linear(3584, 3584, bias=False)
    ...
    def forward(self, features, point_features, targets=None):
        """
        See :class:`ReIDHeads.forward`.
        """
        pool_feat = self.pool_layer(features)
        # 拼接RGB图像和点云特征向量
        pool_feat = torch.cat((pool_feat, point_features), 1)

        neck_feat = self.bottleneck(pool_feat)
        neck_feat = neck_feat[..., 0, 0]

        # 将RGB图像和点云特征融合，加入全连接层
        neck_feat = F.relu(self.fc(neck_feat))
