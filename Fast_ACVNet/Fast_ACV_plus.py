...
from Fast_ACVNet.submodule import *
import math
import gc
import time
import timm

from fastreid.modeling.backbones.mobilenetv3 import _mobilenet_v3_conf, MobileNetV3
...
class Feature(SubModule):
    def __init__(self):
        ...
        inverted_residual_setting, last_channel = _mobilenet_v3_conf('Large', {})
        model = MobileNetV3('BN', inverted_residual_setting, last_channel).features

        layers = [1, 2, 4, 7, 13, 16]
        self.conv_stem = model[0][0]
        self.bn1 = model[0][1]
        self.act1 = model[0][2]

        self.block0 = model[layers[0]:layers[1]]    # 1---16
        self.block1 = model[layers[1]:layers[2]]    # 2,3---24
        self.block2 = model[layers[2]:layers[3]]    # 4,5,6---40
        self.block3 = model[layers[3]:layers[4]]    # 7,8,9,10 11,12---80，112
        self.block4 = model[layers[4]:layers[5]]    # 13,14,15---160
...
class hourglass(nn.Module):
    def __init__(self, in_channels):
        ...
        self.feature_att_8 = channelAtt(in_channels * 2, 80)
        self.feature_att_16 = channelAtt(in_channels * 4, 224)
        self.feature_att_up_8 = channelAtt(in_channels * 2, 80)
...
class hourglass_att(nn.Module):
    def __init__(self, in_channels):
        ...
        self.feature_att_up_16 = channelAtt(in_channels * 4, 224)
        self.feature_att_up_8 = channelAtt(in_channels * 2, 80)
...
class Fast_ACVNet_plus(nn.Module):
    ...
    def forward(self, left, right):
        ...
        if not self.att_weights_only:
            ...
            concat_volume = self.concat_volume_generator(concat_features_left, concat_features_right,
                                                         disparity_sample_topk)
...
class Fast_ACVNet_plus_head(nn.Module):
    def __init__(self, maxdisp, att_weights_only):
        super(Fast_ACVNet_plus_head, self).__init__()
        self.maxdisp = maxdisp
        self.att_weights_only = att_weights_only
        self.feature_up = FeatUp()
        chans = [16, 24, 32, 96, 160]
    ...
    def forward(self, left, right, features_left, features_right):
        features_left, features_right = self.feature_up(features_left, features_right)
        stem_2x = self.stem_2(left)
        stem_4x = self.stem_4(stem_2x)
        stem_2y = self.stem_2(right)
