...
def iou(box_a, box_b, metric='iou_3d'):
    ...
    # # 新增EIOU_3D实现方式
    # if 'eiou_3d' in metric:  # 相比原baseline，有提升
    #     eiou = compute_EIoU(corners1, corners2)
    #     return eiou

    if 'eiou_3d' in metric:  # 相比原baseline，有提升
        box_a_list = Box3D.bbox2array(box_a)
        box_b_list = Box3D.bbox2array(box_b)
        center_distance = np.linalg.norm(box_a_list[:3] - box_b_list[:3], ord=2) # 计算中心距离
        lwh_ry_distance = np.linalg.norm(box_a_list[3:] - box_b_list[3:], ord=1) # 计算长宽高和角度距离
    ...
    if '2d' in metric:  # return 2D IoU/GIoU
        ...
        if metric == 'eiou_3d':
            alpha = 0.08
            return I_3D / U_3D - alpha*(center_distance + lwh_ry_distance)
...
def compute_EIoU(box1, box2, fc=2.0):
    """
    计算两个三维框的 EIoU

    参数：
    box1: (8,3) array in rect camera coord, 第一个三维框的顶点坐标
    box2: (8,3) array in rect camera coord, 第二个三维框的顶点坐标
    fc: float, focal length of camera (默认为2.0)
    ...
    # 计算框的中心点距离
    center_dist = np.linalg.norm(np.mean(box1, axis=0) - np.mean(box2, axis=0))

    # 计算框的长度比例
    dim_ratio = np.prod(np.max(box1, axis=0) - np.min(box1, axis=0)) / np.prod(
        np.max(box2, axis=0) - np.min(box2, axis=0))

    # 计算 EIoU
    EIoU = IoU - np.log((center_dist ** 2 + 1) / (dim_ratio + 1)) / fc
...
def compute_IoU(box1, box2):
    ...
    # 计算两个框的交集和并集的体积
    intersection_vol = intersect_vol(box1, box2)
    union_vol = np.prod(np.max(np.vstack([box1, box2]), axis=0) - np.min(np.vstack([box1, box2]), axis=0))
...
def intersect_vol(box1, box2):
    ...
    返回：
    intersection_vol: float，交集体积

    """

    # 计算两个框的交集体积
    min_coords = np.maximum(np.min(box1, axis=0), np.min(box2, axis=0))
    max_coords = np.minimum(np.max(box1, axis=0), np.max(box2, axis=0))
    overlap_dims = np.maximum(max_coords - min_coords, 0)
    intersection_vol = np.prod(overlap_dims)

    return intersection_vol


def feature_similarity(detection_feature, track_feature):
    feat_similarity = torch.cosine_similarity(detection_feature, track_feature)
