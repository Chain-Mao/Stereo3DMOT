...
from AB3DMOT_libs.box import read_obj_calibration
...
def get_subfolder_seq(dataset, split):
    ...
    if dataset == 'KITTI':  # KITTI
        ...
        if split == 'train': seq_eval = ['0000', '0001', '0002', '0003', '0004', '0005', '0006', '0007', '0008', '0009',
                                         '0010', '0011', '0012', '0013', '0014', '0015', '0016', '0017', '0018', '0019', '0020']  # train
        # if split == 'val':   seq_eval = ['0001', '0006', '0008', '0010', '0012', '0013', '0014', '0015', '0016', '0018', '0019']    # val
        if split == 'val': seq_eval = ['0001', '0004', '0005', '0006', '0009', '0010', '0011', '0012', '0013', '0016', '0019']
        ...
        data_root = os.path.join(file_path,
                                 '../data/nuScenes/nuKITTI')  # path containing the nuScenes-converted KITTI root
...
def initialize(cfg, data_root, save_dir, subfolder, seq_name, cat, ID_start, hw, log_file):
    ...
    # load calibration
    calib_path = os.path.join(data_root, subfolder, 'calib', seq_name + '.txt')
    calib = Calibration(calib_path)
    calib_reid = read_obj_calibration(calib_path)
    # load image for visualization
    img_seq = os.path.join(data_root, subfolder, 'image_02', seq_name)
    img_right_seq = os.path.join(data_root, subfolder, 'image_03', seq_name)
    vis_dir = os.path.join(save_dir, 'vis_debug', seq_name);
    mkdir_if_missing(vis_dir)

    # initiate the tracker
    if cfg.num_hypo > 1:
        tracker = AB3DMOT_multi(cfg, cat, calib=calib, oxts=imu_poses, img_dir=img_seq, img_right_dir=img_right_seq,
                                vis_dir=vis_dir, hw=hw, log=log_file, ID_init=ID_start)
    elif cfg.num_hypo == 1:
        tracker = AB3DMOT(cfg, cat, calib=calib, calib_reid=calib_reid, oxts=imu_poses, img_dir=img_seq,
                          img_right_dir=img_right_seq, vis_dir=vis_dir, hw=hw, log=log_file, ID_init=ID_start)
    else:
        assert False, 'error'
